 ...
 *
 * 在「即将把用户内容发给 AI」的入口处（任务会话页、新建任务页）首次弹出，用户明确点「Agree and continue」后
 * 才放行；持久化到 AsyncStorage，之后不再打扰。点「Not now」则退出该页（不进行任何 AI 交互）。
 */
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onDecline} statusBarTranslucent>
      ...
      <View style={{ position: 'absolute', left: 26, right: 26, top: 0, bottom: 0, alignItems: 'center', justifyContent: 'center' }} pointerEvents="box-none">
        <View style={{ width: '100%', backgroundColor: t.bg2, borderRadius: 22, borderWidth: 1, borderColor: t.line2, padding: 22, ...t.shLift }}>
          <Text style={{ color: t.tx, fontSize: 18, fontWeight: '700', marginBottom: 12 }}>Use AI Coding Assistant</Text>
          <Text style={{ color: t.tx2, fontSize: 14, lineHeight: 22 }}>
            To provide AI coding assistance, content you submit in tasks (including prompts, code, files, and images) may be sent to AI models and third-party AI service providers for processing. By selecting “Agree and continue”, you acknowledge and agree to this data processing.
          </Text>
          <Pressable onPress={openPrivacy} hitSlop={6} style={{ marginTop: 10 }}>
            <Text style={{ color: t.acTx, fontSize: 13, fontWeight: '600' }}>View Privacy Policy</Text>
          </Pressable>
          <Pressable onPress={onAgree} style={({ pressed }) => [{ marginTop: 20, backgroundColor: t.ac, borderRadius: 14, paddingVertical: 14, alignItems: 'center' }, pressed && { opacity: 0.85 }]}>
            <Text style={{ color: t.acInk, fontSize: 15, fontWeight: '700' }}>Agree and continue</Text>
          </Pressable>
          <Pressable onPress={onDecline} style={{ marginTop: 8, paddingVertical: 12, alignItems: 'center' }}>
            <Text style={{ color: t.tx3, fontSize: 14, fontWeight: '600' }}>Not now</Text>
          </Pressable>
        </View>
      </View>
    </Modal>
